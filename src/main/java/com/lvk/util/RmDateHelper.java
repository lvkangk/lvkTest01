...
public class RmDateHelper {
	...
	public static Timestamp getTimestamp(String strDate) {
		...
		try {
			...
			e.printStackTrace();
			//RmLogHelper.error(RmDateHelper.class, "getTimestamp(" + strDate + ") failed");
			return null;
		}
	}
    ...
    public static Timestamp getTomorrow(Timestamp time){
    ...
    public static String parseToTimeDesciption(long timeL) {
    	StringBuilder sb = new StringBuilder();
    	if(timeL / (365 * 24 * 60 * 60 * 1000) > 0) {
    		sb.append(timeL / (365 * 24 * 60 * 60 * 1000) + "年");
    		timeL = timeL % (365 * 24 * 60 * 60 * 1000);
    	}
    	if(timeL / (24 * 60 * 60 * 1000) > 0) {
    		sb.append(timeL / (24 * 60 * 60 * 1000) + "天");
    		timeL = timeL % (24 * 60 * 60 * 1000);
    	}
    	if(timeL / (60 * 60 * 1000) > 0) {
    		sb.append(timeL / (60 * 60 * 1000) + "小时");
    		timeL = timeL % (60 * 60 * 1000);
    	}
    	if(timeL / (60 * 1000) > 0) {
    		sb.append(timeL / (60 * 1000) + "分钟");
    		timeL = timeL % (60 * 1000);
    	}
    	if(sb.length() == 0) {
    		sb.append(timeL / 1000 + "秒");
    	}
    	return sb.toString();
    }
}